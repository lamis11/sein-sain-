...
class config {
  ...
  public static function getConnexion() {
	if (!isset(self::$pdo)) {
	  try{
		self::$pdo = new PDO('mysql:host=localhost;dbname=projet_sain', 'root', '', [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC]);
	...
	class chambreController
	{
		...
		function afficherchambre(){
			$db = config::getConnexion();
			$sql="SELECT * FROM chambre order by date2 desc ";
			$liste=$db->query($sql);
			return $liste;
